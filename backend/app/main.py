    ...
    def _resume_action(
        *,
        ...
        if source_message is None or source_message.role != "assistant":
            raise HTTPException(
                status_code=400, detail="Only assistant action proposals can be resumed"
            )
        ...
        if action.get("status") != "pending":
            raise HTTPException(
                status_code=409, detail=f"Action already {action.get('status')}"
            )
        ...
        if isinstance(async_task_id, int):
            from .services.async_agents import TaskNotResumableError

            try:
                active_agent_service.resume_async_agent(
                    async_task_id,
        ...
        if (
            active_agent_service.settings.feature_workflow_routing
            ...
            fallback_used = False
            try:
                resolved = active_agent_service.normalize_model_selection(originating)
            except ValueError:
                resolved = active_agent_service.default_model_selection
                fallback_used = True
                logger.warning(
            ...
            try:
                execution = active_agent_service.invoke_workflow_resume(
                    session,
                    cmd,
                ...
                raise HTTPException(
                    status_code=502, detail=f"Agent resume failed: {exc}"
                ) from exc
            ...
            if execution.artifact is None:
                ...
                pending = pending_actions_from_interrupts(
                    ...
                    persona=persona,
                    source_meta=active_agent_service._task_source_meta(
                        execution,
                        envelope_final=execution.envelope,
                    ),
                )
            else:
                response_content = active_agent_service._artifact_response_text(
        ...
        if (
            ...
            agent = active_agent_service._sync_agent_for_selection(
                resolved,
                yolo_mode=yolo_mode,
            )
            if agent is None:
                raise HTTPException(
                    status_code=503, detail="Agent is disabled (no LLM configured)"
                )
            ...
            if agent_session is None:
                raise HTTPException(
                    status_code=409, detail=f"AgentSession {session_id} not found"
                )
            thread = session.query(AgentThread).filter(AgentThread.id == thread_id).one()
            ...
            new_msg = active_agent_service._persist_workflow_orchestrator_result(
                session,
                thread,
                result,
                assets=[],
        ...
        thread = session.query(AgentThread).filter(AgentThread.id == thread_id).one()
        new_msg = active_agent_service._persist_agent_result(
            session,
            thread,
            result,
            assets=[],
